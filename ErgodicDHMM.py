...
def K_MeansClustering(dataMatrix,K):
	...
	#randomKMeans=random.sample(range(0,N),K)
	for i in range(K):
	...
	energy=100
	counter=0
	while energy>60 and counter < 50:
		tempClusterMean=[[0 for i in range(dimension)] for i in range(K)]
		for i in range(K):
...
for i in range(len(dataTest)):
	for j in range(len(dataTest[i])):
		ind = maxi_index(prob(dataTest[i][j],symbol_Test[i][j],Lambda[0]),prob(dataTest[i][j],symbol_Test[i][j],Lambda[1]),prob(dataTest[i][j],symbol_Test[i][j],Lambda[2]))
